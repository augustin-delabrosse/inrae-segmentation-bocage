...
def SegFormer():
    id2label = {0:"background", 1:"hedges"}
    id2label = {int(k): v for k, v in id2label.items()}
    label2id = {v: k for k, v in id2label.items()}

    # define model
    segformer = SegformerForSemanticSegmentation.from_pretrained("nvidia/mit-b0",
                                                             num_labels=2,#150,
                                                             id2label=id2label,
                                                             label2id=label2id,
    )
...
class FocalTverskyLoss(nn.Module):
    def __init__(self, add_sigmoid=False, weight=None, size_average=True, alpha=0.5, beta=0.5, gamma=1):
    ...
    def forward(self, inputs, targets, smooth=1):
        ...
        inputs = inputs.view(-1) # logits.view(-1)
